...
- **Single Stock** — browse the universe or search an arbitrary Yahoo Finance ticker (`tab_browse` / `tab_search`), with a price chart overlaying the selected indicators.
...
Pure-numeric portfolio math: `fit_parameters` (drift/covariance from historical returns), `run_monte_carlo` (simulate terminal portfolio value distribution), plus supporting metric/risk-contribution helpers. No Streamlit or I/O — safe to unit test in isolation.
...
A separate, self-contained vectorbt-based backtesting engine (strategies, `backtest.py`, `generate_signals.py`, the old Flask `dashboard.py`, and their tests) lives in this subfolder with its own `requirements.txt`, `config.py`, `DataLoader.py`, and `tickers.txt`. It is no longer wired into the live app — kept only as a reference/starting point for future backtesting work and can be moved out to its own project. See its `README.md` for usage.
